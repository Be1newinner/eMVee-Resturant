...
export default function RegisterScreen() {
    ...
    try {
      await createUserWithEmailAndPassword(firebaseAuth, email, password).then(
        async ({ user }) => {
          ...
          await updatePhoneNumber({ uid: user.uid, phone });
          ...
          if (!user?.emailVerified) {
            sendEmailVerification(user).then(async () => {
              ...
              cleanUp();
              navigation.navigate({
                pathname: PAGES_STACK.VERIFY_EMAIL,
                params: {
                  email: emailID,
                  password: password,
                },
      ...
      if (error.toString().includes("invalid-credential")) {
        ...
        setError({ other: "Email Already Exist!" });
      } else {
        setError({ other: "Error! Please try again!" });
      }
      setIsLoading(false);
    }
  };
  ...
  const updatePhoneNumber = async ({ uid, phone }) => {
    try {
      await setDoc(doc(firestoreDB, "us7", uid), {
        p: phone,
      });
    } catch (e) {
  ...
  const validation = () => {
    ...
    if (!phone) error.phone = "Contact Number is required!";
    else if (phone.length != 10)
      error.phone = "Contact Number must be 10 digit!";

    if (!name) error.name = "Name is required!";
    else if (name.length < 3) error.phone = "Invalid Name.";

    if (error.email || error.password || error.name || error.phone) {
      setError(error);
      return false;
    } else return true;
  };
  ...
  const signIn = () => {
    if (validation()) registerUser({ email: emailID, password, phone, name });
  };
      ...
      >
        ...
        >
          ...
          <Input
            ...
            accessoryLeft={() => <Entypo name="user" size={24} color="black" />}
          ...
          >
            ...
            <Text
              status="primary"
              onPress={() => navigation.replace(PAGES_STACK.LOGIN_SCREEN)}
            >
              Log in
            </Text>
          </Text>
